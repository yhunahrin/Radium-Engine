...
namespace Ra
{
    namespace Gui
    {
            ...
            , m_selectedAxis(-1)
        {
            ...
            Ra::Engine::AssetManager* mgr = Ra::Engine::AssetManager::getInstance();
            ...
            for (uint i = 0; i < 3; ++i)
            {
                ...
                Core::TriangleMesh cylinder =
                    Core::MeshUtils::makeCylinder(Core::Vector3::Zero(),
                                                  arrowScale * cylinderEnd,
                                                  arrowScale * axisWidth / 2.f);

                Core::TriangleMesh cone = Core::MeshUtils::makeCone(arrowScale * cylinderEnd,
                                                                    arrowScale * arrowEnd,
                ...
                Ra::Engine::Mesh* mesh = mgr->mesh(mgr->createMesh("Gizmo Arrow"));
                ...
                auto drawable = RenderObject::createRenderObject("Gizmo Arrow",
                                                                 m_comp,
                ...
                m_renderObjects.push_back(m_comp->addRenderObject(drawable));
            }
        }

        void TranslateGizmo::updateTransform(const Core::Transform& worldTo,
                                             const Core::Transform& t)
        {
            m_worldTo                        = worldTo;
            m_transform                      = t;
        ...
        void TranslateGizmo::selectConstraint(int drawableIdx)
        {
            ...
            if (drawableIdx >= 0)
            {
                auto found = std::find(m_renderObjects.cbegin(),
                                       m_renderObjects.cend(),
                                                  ...
                                                  const Core::Vector2&  nextXY)
        {
            if (m_selectedAxis >= 0)
            {
                ...
                Core::Vector3       translateDir =
                    m_mode == LOCAL ? Core::Vector3(m_transform.rotation() *
                                                    Core::Vector3::Unit(m_selectedAxis))
                                    : Core::Vector3::Unit(m_selectedAxis);

                if (!m_start)
                {
                    if (findPointOnAxis(cam,
                                        origin,
                                        translateDir,
                                        m_initialPix + nextXY,
                                        m_startPoint))
