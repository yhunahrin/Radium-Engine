...
namespace Ra
{
    namespace Gui
    {
        ...
        void MaterialEditor::onExpChanged(double v)
        {
            if (m_renderObject)
            {
                m_renderObject->renderTechnique->material->m_ns = v;
            }
        }
        ...
        void MaterialEditor::onKdColorChanged(int)
        {
            ...
            if (m_renderObject)
            {
                Core::Color c(kdR->value() / 255.f,
                              kdG->value() / 255.f,
                              kdB->value() / 255.f,
                              1.0);
                m_renderObject->renderTechnique->material->m_kd = c;
            }
        }
        ...
        void MaterialEditor::onKsColorChanged(int)
        {
            ...
            if (m_renderObject)
            {
                Core::Color c(ksR->value() / 255.f,
                              ksG->value() / 255.f,
                              ksB->value() / 255.f,
                              1.0);
                m_renderObject->renderTechnique->material->m_ks = c;
            }
        }
        ...
        void MaterialEditor::changeRenderObject(Core::Index roIdx)
        {
            ...
            if (m_renderObject != nullptr)
            {
                ...
                updateMaterialViz(m_renderObject->renderTechnique->material);
            }
        }
