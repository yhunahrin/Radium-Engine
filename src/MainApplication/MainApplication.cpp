...
namespace Ra
{
        ...
        , m_isAboutToQuit(false)
    {
        ...
        // For any reason, the third parameter must be set if you want to be able to read anything
        // from it (and it cannot be "")
        QCommandLineOption fpsOpt(QStringList{"r", "framerate", "fps"},
                                  "Control the application framerate, 0 to disable it (and run as "
                                  "fast as possible)",
                                  "60");
        ...
        config.str(std::string());
        config << "build: " << Version::compiler << " - " << Version::compileDate << " "
               << Version::compileTime;
    ...
    {
        connect(m_mainWindow.get(),
                &Gui::MainWindow::closed,
                this,
                &MainApplication::appNeedsToQuit);
    }
    ...
    void MainApplication::setupScene()
    {
        ...
        Engine::SystemEntity::uiCmp()->addRenderObject(Primitive(Engine::SystemEntity::uiCmp(),
        ...
        Engine::SystemEntity::uiCmp()->addRenderObject(
            Primitive(Engine::SystemEntity::uiCmp(),
    ...
    void MainApplication::radiumFrame()
    {
        ...
        const Scalar dt = m_realFrameRate ? Core::Timer::getIntervalSeconds(m_lastFrameStart,
    ...
    void MainApplication::setRealFrameRate(bool on) { m_realFrameRate = on; }

    void MainApplication::setRecordFrames(bool on) { m_recordFrames = on; }
