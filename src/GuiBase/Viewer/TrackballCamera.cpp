...
namespace Ra {
  ...
  Gui::TrackballCamera::TrackballCamera(uint width, uint height)
      : CameraInterface(width, height), m_trackballCenter(0, 0, 0), m_quickCameraModifier(1.f),
        m_wheelSpeedModifier(0.02f), m_distFromCenter(1.0f), m_cameraRadius(1.0f), m_rotateAround(true),
        m_cameraRotateMode(false), m_cameraPanMode(false), m_cameraZoomMode(false), m_walkingOn(false),
        m_strafingOn(false), m_climbingOn(false), m_walking(0), m_strafing(0), m_climbing(0)
  {
      resetCamera();
  }
  ...
  bool Gui::TrackballCamera::handleMousePressEvent(QMouseEvent *event)
  {
      ...
      if (!(Gui::KeyMappingManager::getInstance()->actionTriggered(event,
                                                                   Gui::KeyMappingManager::TRACKBALLCAMERA_MANIPULATION)))
      {
          return false;
      }
  ...
  {
      if (Gui::KeyMappingManager::getInstance()->actionTriggered(e,
                                                                 Gui::KeyMappingManager::TRACKBALLCAMERA_ROTATE_AROUND))
      {
