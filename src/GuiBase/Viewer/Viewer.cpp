...
namespace Ra {
  Gui::Viewer::Viewer(QScreen *screen)
      : QWindow(screen), m_context(nullptr), m_currentRenderer(nullptr), m_featurePickingManager(nullptr),
        m_camera(nullptr), m_gizmoManager(nullptr), m_renderThread(nullptr), m_glInitStatus(false)
  {
...
//        LOG( logDEBUG ) << "Gui::Viewer::initializeGL : "  << width() << 'x' << height() << std::endl;
  ...
  void Gui::Viewer::keyReleaseEvent(QKeyEvent *event)
  {
      ...
      if (Gui::KeyMappingManager::getInstance()->actionTriggered(event, Gui::KeyMappingManager::VIEWER_TOGGLE_WIREFRAME)
          && !event->isAutoRepeat())
      {
  ...
  void Gui::Viewer::processPicking()
  {
      ...
      for (uint i = 0; i < m_currentRenderer->getPickingQueries().size(); ++i)
      {
          ...
          else if (query.m_button == Core::MouseButton::RA_MOUSE_RIGHT_BUTTON)
          {
              ...
              const Core::Ray ray = m_camera->getCamera()->getRayFromScreen({query.m_screenCoords(0),
                                                                             height() - query.m_screenCoords(1)});
