...
namespace Ra {
  namespace Gui {
    ...
    void FeaturePickingManager::doPicking(int roIndex,
                                          const Engine::Renderer::PickingQuery &query,
                                          const Core::Ray &ray)
    {
    ...
    void FeaturePickingManager::setVertexIndex(int id)
    {
        ...
        {
            auto ro =
                Ra::Engine::RadiumEngine::getInstance()->getRenderObjectManager()->getRenderObject(m_FeatureData.m_roIdx);
