...
/// CONSTRUCTOR
    AssimpAnimationDataLoader::AssimpAnimationDataLoader(const bool VERBOSE_MODE) : DataLoader<Asset::AnimationData>(
        VERBOSE_MODE)
    {}
...
/// LOADING
    void AssimpAnimationDataLoader::loadData(const aiScene *scene,
                                             std::vector<std::unique_ptr<Asset::AnimationData> > &data)
    {
        data.clear();
...
/// KEY FRAME
    void AssimpAnimationDataLoader::loadAnimationData(const aiScene *scene,
