...
namespace Ra {
  namespace IO {
    ...
    Asset::FileData *TinyPlyFileLoader::loadFile(const std::string &filename)
    {
        ...
        geometry->setVertices(*(reinterpret_cast<std::vector<Eigen::Matrix<float,
                                                                           3,
                                                                           1,
                                                                           Eigen::DontAlign>> *> (&verts)));
        geometry->setFrame(Core::Transform::Identity());

        if (normalCount != 0)
        {
            geometry->setNormals(*(reinterpret_cast<std::vector<Eigen::Matrix<float,
                                                                              3,
                                                                              1,
                                                                              Eigen::DontAlign>> *> (&normals)));
        }

        if (colorCount != 0)
        {
            geometry->setColors(*(reinterpret_cast<std::vector<Eigen::Matrix<uint8_t,
                                                                             4,
                                                                             1,
                                                                             Eigen::DontAlign>> *> (&colors)));
