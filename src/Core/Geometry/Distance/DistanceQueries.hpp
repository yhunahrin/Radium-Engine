...
namespace Ra {
  namespace Core {
    namespace DistanceQueries {
      ...
      inline RA_CORE_API Scalar
      pointToLineSq(const Vector3 &q, const Vector3 &a, const Vector3 &dir);
      ...
      inline RA_CORE_API Scalar
      projectOnSegment(const Vector3 &q, const Vector3 &a, const Vector3 &ab);

      /// Return the squared distance from point Q to the segment AB defined by point A and
      /// vector AB = (B - A).
      inline RA_CORE_API Scalar
      pointToSegmentSq(const Vector3 &q, const Vector3 &a, const Vector3 &ab);
      ...
      struct PointToTriangleOutput
      {
          ...
          enum Flags
          {
              HIT_FACE = 0, HIT_VERTEX = 1, HIT_EDGE = 2
          };
          ...
          inline Flags getHitPrimitive() const
          { return Flags(flags & 0x3u); }
      ...
      lineToSegSq(const Vector3 &lineOrigin,
