...
//    typedef std::vector<Index>      IndexList;
      ...
      inline const NodeList &_getNodes(void)
      { return mNodes; }
      inline const PointList &_getPoints(void)
      { return mPoints; }
      inline const TriangleList &_getTriangles(void)
      { return mTriangles; }
      inline const IndexList &_getIndices(void)
      { return mIndices; }
      ...
      inline const AxisAlignedBoxType &aabb() const
      { return mAABB; }
      ...
      inline void doQueryRestrictedClosestIndexes(const VectorType &s1,
                                                  const VectorType &s2,
                                                  Scalar sqdist,
                                                  std::vector <Index> &cl_dist);
...
//                }
                  ...
                  for (int i = 0; i < nbTriangles; i++)
                  {
                      ...
                      const Scalar sqdist = Ra::Core::DistanceQueries::segmentToTriSq(segCenter,
                                                                                      segDirection,
                                                                                      segExtent,
                                                                                      triangle).sqrDistance;
...
//            }
              ...
              if (node.leaf)
              {
                  ...
                  for (int i = 0; i < nbTriangles; i++)
                  {
                      ...
                      const VectorType triangle[3] = {mPoints[mTriangles[node.triangleIndices[i]][0]],
                                                      mPoints[mTriangles[node.triangleIndices[i]][1]],
                                                      mPoints[mTriangles[node.triangleIndices[i]][2]]};
                      const Scalar dist = Ra::Core::DistanceQueries::segmentToTriSq(segCenter,
                                                                                    segDirection,
                                                                                    segExtent,
                                                                                    triangle).sqrDistance;
                      if (dist < sqdist)
  ...
  template<typename Index>
  void TriangleKdTree<Index>::createTree(unsigned int nodeId, /*IndexList& triangleIndices,*/ /*unsigned int leftchild, AxisAlignedBoxType bb, unsigned int d,*/
                                         float splitValue,
                                         unsigned int level,
                                         unsigned int targetCellSize,
                                         unsigned int targetMaxDepth)
...
//    }
          ...
          if (matches > 0 && (mNodes[node.firstChildId].triangleIndices.size() / matches <= 1
...
//            child.leafTriangleIndices = child.triangleIndices;
                  ...
                  {
                      child.leaf = 0;
                      createTree(childId, /*child.triangleIndices,*/ /*1, aabb, node.dim,*/
                                 node.splitValue,
                                 level + 1,
                                 targetCellSize,
                                 targetMaxDepth);
...
//            child.leafTriangleIndices = child.triangleIndices;
                  ...
                  {
                      child.leaf = 0;
                      createTree(childId, /*child.triangleIndices,*/ /*0, aabb, node.dim,*/
                                 node.splitValue,
                                 level + 1,
                                 targetCellSize,
                                 targetMaxDepth);
