...
namespace Ra {
  namespace Core {
    ...
    namespace Colors {
      ...
      template<typename C = Color> inline C Alpha()
      { return C(0.0, 0.0, 0.0, 0.0); }
      template<typename C = Color> inline C Black()
      { return C(0, 0, 0, 1); }

      template<typename C = Color> inline C Red()
      { return C(1, 0, 0, 1); }
      template<typename C = Color> inline C Green()
      { return C(0, 1, 0, 1); }
      template<typename C = Color> inline C Blue()
      { return C(0, 0, 1, 1); }

      template<typename C = Color> inline C Yellow()
      { return C(1, 1, 0, 1); }
      template<typename C = Color> inline C Magenta()
      { return C(1, 0, 1, 1); }
      template<typename C = Color> inline C Cyan()
      { return C(0, 1, 1, 1); }

      template<typename C = Color> inline C White()
      { return C(1, 1, 1, 1); }

      template<typename C = Color> inline C Grey(Scalar f = 0.5f)
      { return Color(f, f, f, 1); }

      template<typename C = Color> inline C Skin()
      ...
      inline uint32_t ToRGBA32(const C &color)
      {
          ...
          return (uint32_t(scaled[0]) << 24) | (uint32_t(scaled[1]) << 16) | (uint32_t(scaled[2]) << 8)
              | (uint32_t(scaled[3]) << 0);
      }
      ...
      inline uint32_t ToARGB32(const C &color)
      {
          C c(color * 255);
          Vector4i scaled(c.x(), c.y(), c.z(), c.w());
          return (uint32_t(scaled[3]) << 24) | (uint32_t(scaled[0]) << 16) | (uint32_t(scaled[1]) << 8)
              | (uint32_t(scaled[2]) << 0);
      }
