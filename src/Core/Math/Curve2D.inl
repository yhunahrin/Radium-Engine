...
namespace Ra {
  namespace Core {
    ...
    Curve2D::Vector CubicBezier::fdf(Scalar t, Vector &grad) const
    {
        ...
        grad = 3.0 * oneMinusT2 * (m_points[1] - m_points[0]) + 6.0 * oneMinusT * t * (m_points[2] - m_points[1])
            + 3.0 * t2 * (m_points[3] - m_points[2]);
        return oneMinusT3 * m_points[0] + 3.0 * oneMinusT2 * t * m_points[1] + 3.0 * oneMinusT * t2 * m_points[2]
            + t3 * m_points[3];
    }
