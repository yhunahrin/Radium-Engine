...
namespace Ra {
  namespace Core {
    ...
    void TaskQueue::addDependency(TaskQueue::TaskId predecessor, TaskQueue::TaskId successor)
    {
        ...
        CORE_ASSERT(std::find(m_dependencies[predecessor].begin(), m_dependencies[predecessor].end(), successor)
                        == m_dependencies[predecessor].end(), "Cannot add a dependency twice");
    ...
    void TaskQueue::resolveDependencies()
    {
        for (const auto &pre : m_pendingDepsPre)
        {
            ...
            CORE_WARN_IF(!result,
                         "Pending dependency unresolved : " << m_tasks[pre.first]->getName() << " -> (" << pre.second
                                                            << ")");
        }
        for (const auto &pre : m_pendingDepsSucc)
        {
            ON_ASSERT(bool result =) addDependency(pre.first, pre.second);
            CORE_WARN_IF(!result,
                         "Pending dependency unresolved : (" << pre.first << ") -> " << m_tasks[pre.second]->getName());
        }
    ...
    void TaskQueue::printTaskGraph(std::ostream &output) const
    {
        ...
        for (const auto &preDep :m_pendingDepsPre)
        {
            ...
            if (std::find_if(m_tasks.begin(), m_tasks.end(),
                             [=](const auto &task)
                             { return task->getName() == t2name; }) == m_tasks.end())
            {
                t2name += "?";
            }
        ...
        for (const auto &postDep :m_pendingDepsSucc)
        {
            ...
            if (std::find_if(m_tasks.begin(), m_tasks.end(),
                             [=](const auto &task)
                             { return task->getName() == t1name; }) == m_tasks.end())
            {
                t1name += "?";
            }
