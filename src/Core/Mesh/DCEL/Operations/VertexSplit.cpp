    ...
    namespace Core {
        namespace DcelOperations {
            ...
            void vertexSplit(Dcel& dcel, ProgressiveMeshData pmdata)
            {
                ...
                // Manage T configuration
                if (pmdata.getTConfig().getVTConfigId() != -1)
                {
                    HalfEdge_ptr heT = dcel.m_halfedge[pmdata.getTConfig().getHeTConfigId()];
                    Vertex_ptr veT = dcel.m_vertex[pmdata.getTConfig().getVTConfigId()];
                    Face_ptr f1TConfig = dcel.m_face[pmdata.getTConfig().getF1TConfigId()];
                    Face_ptr f2TConfig = dcel.m_face[pmdata.getTConfig().getF2TConfigId()];

                    veT->setP(pmdata.getTConfig().getVTConfigPos());
                    veT->setHE(heT);

                    f1TConfig->setHE(heT);
                    f2TConfig->setHE(heT->Twin());

                    Vertex_ptr v0 = dcel.m_face[flId]->HE()->V();
                    Vertex_ptr v1 = dcel.m_face[flId]->HE()->Next()->V();
                    Vertex_ptr v2 = dcel.m_face[flId]->HE()->Prev()->V();
                    Vector3 barCoord = Geometry::barycentricCoordinate(pmdata.getTConfig().getVTConfigPos(), v0->P(), v1->P(), v2->P());
                    if (barCoord[0] > 0 && barCoord[0] < 1 &&
                        barCoord[1] > 0 && barCoord[1] < 1 &&
                        barCoord[2] > 0 && barCoord[2] < 1) //T-config in Fl
                    {
                        heT->Twin()->Prev()->setV(vt);

                        heFlPrev->setV(veT);
                        heFlPrev->setTwin(heT->Prev());
                        heFlclwOpCurr->setTwin(heT->Twin()->Prev());

                        heFlNext->setTwin(heT->Twin()->Next());
                        heFlclwCurr->setTwin(heT->Next());
                    ...
                    else
                    {
                        ...
                        heFrPrev->setV(veT);
                        heFrPrev->setTwin(heT->Prev());
                        heFrcrwOpCurr->setTwin(heT->Next());

                        heFrNext->setTwin(heT->Twin()->Next());
                        heFrcrwCurr->setTwin(heT->Twin()->Prev());
                    }
                }
            }
