...
//#ifndef NO_TRANSPARENCY
        ...
        {
            auto fboSize = getFboSize();
            uint w = fboSize.first;
            uint h = fboSize.second;
            LOG( logERROR ) << w << " " << h;

            m_pingPongSize = std::pow(2.0, Scalar(uint(std::log2(std::min(w, h)))));

            m_textures[RendererTextures_Depth]->Generate(w, h, GL_DEPTH_COMPONENT);
            m_textures[RendererTextures_HDR]->Generate(w, h, GL_RGBA);
            m_textures[RendererTextures_Normal]->Generate(w, h, GL_RGBA);
            m_textures[RendererTextures_Diffuse]->Generate(w, h, GL_RGBA);
            m_textures[RendererTextures_Specular]->Generate(w, h, GL_RGBA);
            m_textures[RendererTextures_OITAccum]->Generate(w, h, GL_RGBA);
            m_textures[RendererTextures_OITRevealage]->Generate(w, h, GL_RGBA);
