...
namespace Ra {
  ...
  namespace Engine {
    ...
    Camera::Camera(Scalar height, Scalar width)
        : m_frame(Core::Transform::Identity()), m_projMatrix(Core::Matrix4::Identity()), m_fov(PiDiv4),
          m_zNear(Scalar(0.1f)), m_zFar(Scalar(1000.0f)), m_zoomFactor(Scalar(1.0f)), m_width(width), m_height(height),
          m_aspect(width / height), m_projType(ProjType::PERSPECTIVE)
    {
    }
