...
namespace Ra
{
    namespace Engine
    {
        ...
        Renderer::Renderer( )
            : m_width( 0 )
            , m_height( 0 )
            , m_fboWidth( std::numeric_limits<uint>::max() )
            , m_fboHeight( std::numeric_limits<uint>::max() )
            , m_shaderMgr( nullptr )
        ...
        void Renderer::resize( uint w, uint h )
        {
            ...
            uint fboW = fboSize.first;
            uint fboH = fboSize.second;

            m_depthTexture->Generate(fboW, fboH, GL_DEPTH_COMPONENT);
            m_pickingTexture->Generate(fboW, fboH, GL_RGBA_INTEGER);
            m_fancyTexture->Generate(fboW, fboH, GL_RGBA);
        ...
        void Renderer::setFBOMaxSize(uint fboWidth, uint fboHeight)
        {
            m_fboWidth = fboWidth;
            m_fboHeight = fboHeight;
        }

        std::pair<uint, uint> Renderer::getFboSize() const
        {
            Scalar ratio = Scalar ( m_width ) / Scalar ( m_height );
            uint w = m_fboWidth;
            uint h = m_fboHeight;

            // Compute FBO size according to current factor ratio
            if ( ratio < Scalar( 1 ) ) // height > width
            {
                w *= ratio;
            }
            else
