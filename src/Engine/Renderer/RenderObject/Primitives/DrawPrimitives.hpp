...
namespace Ra
{
    namespace Engine
    {
        ...
        namespace DrawPrimitives
        {
            ...
            RA_ENGINE_API RenderObject* Primitive(Component* comp, Mesh* mesh);
            ...
            /// Displays given line
            RA_ENGINE_API Mesh* Line(const Core::Vector3& a,
            ...
            RA_ENGINE_API Mesh* Ray(const Core::Ray& ray, const Core::Color& color);
            ...
            RA_ENGINE_API Mesh* Frame(const Core::Transform& frameFromEntity, Scalar scale = 0.1f);
            ...
            RA_ENGINE_API Mesh* AABB(const Core::Aabb& aabb, const Core::Color& color);

            /// Display a wireframe OBB, given an AABB and a transform
            RA_ENGINE_API Mesh* OBB(const Core::Obb& obb, const Core::Color& color);

            RA_ENGINE_API Mesh* Spline(const Core::Spline<3, 3>& spline,
                                       uint               pointCount,
                                       const Core::Color& color,
                                       Scalar             scale = 1.0f);
        }
