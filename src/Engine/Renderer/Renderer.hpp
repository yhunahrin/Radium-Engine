...
namespace Ra
{
    namespace Engine
    {
        ...
        public:
            ...
            /// setFBOMaxSize defines the maximum size allowed for FBOs textures
            ///
            /// By default, FBO size is not limited (i.e., set to std::numeric_limits<uint>::max() )
            void setFBOMaxSize( uint fboWidth, uint fboHeight );

            /// Computes FBO width depending on renderer size and FBO limit
            ///
            /// Returns a pair agregating width (first) and height (second)
            std::pair<uint, uint> getFboSize() const;
