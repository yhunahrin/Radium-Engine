...
namespace AnimationPlugin
{
    ...
    SkeletonBoneRenderObject::SkeletonBoneRenderObject(const std::string&               name,
        ...
        : m_roIdx(Ra::Core::Index::INVALID_IDX())
        , m_id(id)
        ...
        , m_roMgr(roMgr)
    {
        ...
        auto ro = new RenderObject(name, comp, Ra::Engine::RenderObjectType::Fancy);
        ro->setXRay(true);

        auto shader = Ra::Engine::ShaderConfigurationFactory::getConfiguration("BlinnPhong");

        m_material       = mgr->material(mgr->createMaterial("Bone Material"));
        ...
        m_renderParams = mgr->renderTechnique(mgr->createRenderTechnique());
        {
            m_renderParams->shaderConfig = shader;
            m_renderParams->material     = m_material;
        }
        ro->renderTechnique = m_renderParams;
        ...
        m_roIdx = m_roMgr->addRenderObject(ro);
        updateLocalTransform();
    }

    void SkeletonBoneRenderObject::update() { updateLocalTransform(); }
    ...
    Ra::Core::TriangleMesh SkeletonBoneRenderObject::makeBoneShape()
    {
        ...
        mesh.m_vertices = {Ra::Core::Vector3(0, 0, 0),
        ...
        mesh.m_triangles = {Ra::Core::Triangle(0, 2, 3),
                            Ra::Core::Triangle(0, 5, 2),
                            Ra::Core::Triangle(0, 3, 4),
                            Ra::Core::Triangle(0, 4, 5),
                            Ra::Core::Triangle(1, 3, 2),
                            Ra::Core::Triangle(1, 2, 5),
                            Ra::Core::Triangle(1, 4, 3),
                            Ra::Core::Triangle(1, 5, 4)};
