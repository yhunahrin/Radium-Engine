...
namespace AnimationPlugin
{
    ...
    void AnimationComponent::handleSkeletonLoading( const Ra::Asset::HandleData* data, const std::map< uint, uint >& duplicateTable ) {
        ...
        for (int i = 0; i < 3; i++)
        {
            frameTransform.matrix().col(i).normalize();
        }
        
        for (int i = 0; i < m_skel.size(); i++)
        {
            if (m_skel.m_graph.isRoot(i))
               m_skel.setTransform(i, frameTransform.inverse() * m_skel.getTransform(i, Ra::Core::Animation::Handle::SpaceType::LOCAL), Ra::Core::Animation::Handle::SpaceType::LOCAL);
        }
